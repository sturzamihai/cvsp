...
from cvsp.models.flip import third_party

import timm
from cvsp.models.flip import clip
from cvsp.models.flip.prompt_templates import spoof_templates, real_templates
...
class feature_generator_beit(nn.Module):
    ...
    def __init__(self):
        ...
        self.vit = timm.create_model(
            "beitv2_base_patch16_224", pretrained=True
        )  # This will load the v2 of BEiT and the weights will be the pretrained on in1k and intermediate finetuned on 21kto1k
...
class feature_generator_fix_vitaf(nn.Module):
    ...
    def __init__(self):
        ...
        self.vit.head = (
            nn.Identity()
        )  # remove the classification head for timm version 0.6.12
...
class feature_embedder(nn.Module):
    ...
    def __init__(self):
        ...
        self.bottleneck_layer = nn.Sequential(
            self.bottleneck_layer_fc, nn.ReLU(), nn.Dropout(0.5)
        )
...
class flip_it(nn.Module):
    ...
    def __init__(self):
        ...
        # self.text_inputs = torch.cat([clip.tokenize("a photo of a spoof face") , clip.tokenize("a photo of a real face")])
    ...
    def forward(self, input, input_view_1, input_view_2, source_labels, norm_flag=True):
        ...
        for label in source_labels:
            ...
            if label == 0:  # spoof
                ...
                spoof_texts_v1 = torch.stack(spoof_texts_v1, dim=0)  # 3x512
                spoof_texts_v2 = torch.stack(spoof_texts_v2, dim=0)  # 3x512
                assert (
                ...
                # stack the embeddings
                real_texts_v1 = torch.stack(real_texts_v1, dim=0)  # 3x512
                real_texts_v2 = torch.stack(real_texts_v2, dim=0)  # 3x512
                assert (
        ...
        text_embed_v1 = torch.stack(text_embedding_v1, dim=0)  # Bx512
        text_embed_v2 = torch.stack(text_embedding_v2, dim=0)  # Bx512
        assert (
