...
class Config(Validation, validate_assignment=True):
    ...
    throw_exception_if_run_exists: bool = (
        False  # Throw an exception if the run directory exists
    )
    ...
    slerp_feature_augmentation_range: list[float] = [
        0.0,
        1.0,
    ]  # Range of the Slerp feature augmentation

    # Data configuration
    trn_files: list[str] | dict[str, list[str]] = (
        []
    )  # Files containing paths to training samples
    val_files: list[str] | dict[str, list[str]] = (
        []
    )  # Files containing paths to validation samples
    tst_files: list[str] | dict[str, list[str]] = (
        []
    )  # Files containing paths to test samples
    ...
    limit_train_batches: None | int | float = (
        None  # Limit the number of training batches
    )
    limit_test_batches: None | int | float = None  # Limit the number of test batches
    limit_val_batches: None | int | float = (
        None  # Limit the number of validation batches
    )
    ...
    # Post-processing
    make_binary_before_video_aggregation: bool = (
        True  # Make binary labels before video aggregation
    )
