...
from . import metrics, plots, head
from .config import Backbone, Config, Head
from .dataset import BaseDataset
from .loss import Loss, LossInputs, LossOutputs, alignment, uniformity
...
def slerp(A: torch.Tensor, B: torch.Tensor, t: torch.Tensor | float) -> torch.Tensor:
    ...
    dot = torch.sum(A * B, dim=-1, keepdim=True).clamp(
        -1 + 1e-7, 1 - 1e-7
...
class DeepfakeDetectionModel(pl.LightningModule):
    ...
    def _init_feature_extractor(self):
        ...
        if "clip" in backbone or "FaRL" in backbone:
            if Head.needs_patches(self.config.head):
                from .encoder import CLIPEncoderPatches
    ...
    def _init_peft(self):
        if self.config.peft.enabled:
            ...
            if self.config.peft.lora is not None and self.config.peft.lora.enabled:
                ...
                self.config.peft.ln_tuning is not None
                and self.config.peft.ln_tuning.enabled
            ):
                from peft import LNTuningConfig

                peft_config = LNTuningConfig(
                    target_modules=self.config.peft.ln_tuning.target_modules
                )
    ...
    def _init_head(self):
        ...
        match self.config.head:
            ...
            case Head.LinearNorm:
                self.model = head.LinearProbe(
                    features_dim, self.config.num_classes, True
                )
    ...
    def log_aliunif(self, outputs: head.HeadOutput, labels: torch.Tensor, stage: str):
        alignment_out = alignment(outputs.features, labels)
        uniformity_out = uniformity(outputs.features)
        self.log(f"{stage}/alignment", alignment_out, prog_bar=True, on_epoch=True)
        self.log(f"{stage}/uniformity", uniformity_out, prog_bar=True, on_epoch=True)
    ...
    def slerp_feature_augmentation(self, batch: Batch, features: torch.Tensor):
        ...
        if self.training and self.config.slerp_feature_augmentation:
            ...
            for class_label in torch.unique(labels):
                ...
                t = torch.rand(
                    (num_embeddings, 1), device=features.device, dtype=features.dtype
                )
    ...
    def log_metrics(
        ...
        self.log(
            f"{prefix}/balanced_acc_{level}", M.balanced_accuracy_score(labels, preds)
        )
        self.log(
            f"{prefix}/f1_score_{level}", M.f1_score(labels, preds, average="macro")
        )
    ...
    def log_all_metrics(
        ...
        files = [
            dataset.files[i] for i in idx
        ]  # Get files in the same order as the rest
    ...
    def on_test_epoch_end(self):
        ...
        probs = {
            f"prob_class_{i}": np.round(probs[:, i], 4) for i in range(probs.shape[1])
        }
        table = pd.DataFrame({"files": files, "labels": labels, **probs})

        # Save to CSV
        table.to_csv(
            f"{self.logger.log_dir}/test_predictions.csv",
            index=False,
            float_format="%.4f",
        )
        ...
        if self.config.lr_scheduler == "cosine":
            ...
            if self.config.limit_train_batches is not None:
                logger.print_warning_once(
                    "lr scheduling and limit_train_batches are not compatible"
                )
            T_max = self.config.max_epochs * len(self.trainer.train_dataloader)
            scheduler = CosineAnnealingLR(
                optimizer, T_max=T_max, eta_min=self.config.min_lr
            )
