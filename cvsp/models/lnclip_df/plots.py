...
def plot_curve(
    ...
    for c, (x, y) in enumerate(zip(xs, ys)):
        ...
        if auc >= auc_threshold:  # Only plot and include in legend if AUC > threshold
            ...
            line = ax_plot.plot(
                x, y, label=label, linewidth=1.5, color=palette[c], linestyle=linestyle
            )
...
def plot_prc_curve(
    ...
    if show_f1_lines:
        ...
        for f_score in f_scores:
            ...
            ax_plot.annotate(
                "F1={0:0.1f}".format(f_score), xy=(0.95, p[-1] - 0.02), alpha=0.2
            )
...
def plot_confusion_matrix(
    ...
    if normalize:
        confusion_matrix = (
            confusion_matrix / confusion_matrix.sum(axis=1, keepdims=True) * 100
        )
