import torch
from cvsp.models.sac.unet_model import UNet
...
def mask_clustering(mask_ps, num_iters=5, n_patch=3):
    ...
    if len(idx_tuple) > 2:
        ...
        for i in range(n_patch):
            ...
            mask_clustered_i = (
                torch.from_numpy(mask_clustered_i).unsqueeze(0).unsqueeze(0).cuda()
            )
...
def ShapeCompletionL1KnownSquare(mask, size, l1_thresh, soft=False):
    ...
    included_sums[:-size, :-size] = (
        corner_sums[:-size, :-size]
        + corner_sums[size:, size:]
        - corner_sums[:-size, size:]
        - corner_sums[size:, :-size]
    )
    ...
    if soft:
        is_to_be_masked = ThresholdSTEFunction.apply(
            1.5 - l1_dists.float() / (size * size) - l1_thresh
        )
        t = 10 * math.log(100)
        is_to_be_masked_corners = (
            torch.logcumsumexp(torch.logcumsumexp(is_to_be_masked * t, dim=1), dim=0)
            / t
        )
        ...
        out_mask[size:, size:] = (
            torch.exp(t * is_to_be_masked_corners[:-size, :-size] - q1)
            + torch.exp(t * is_to_be_masked_corners[size:, size:] - q1)
            - torch.exp(t * is_to_be_masked_corners[:-size, size:] - q1)
            - torch.exp(t * is_to_be_masked_corners[size:, :-size] - q1)
        )
        ...
        out_mask[size:, :size] = torch.exp(
            t * is_to_be_masked_corners[size:, :size] - q2
        ) - torch.exp(t * is_to_be_masked_corners[:-size, :size] - q2)
        ...
        out_mask[:size, size:] = torch.exp(
            t * is_to_be_masked_corners[:size, size:]
        ) - torch.exp(t * is_to_be_masked_corners[:size, :-size])
        out_mask_2[:size, size:] = (torch.log(out_mask[:size, size:]) + q3) / t
        out_mask_2[:size, :size] = is_to_be_masked_corners[:size, :size]
        return ThresholdSTEGEFunction.apply(out_mask / 2).reshape(
            (1, 1, mask.shape[0], mask.shape[1])
        )
    else:
        is_to_be_masked = ThresholdSTEFunction.apply(
            1.5 - l1_dists.float() / (size * size) - l1_thresh
        ...
        out_mask = torch.zeros_like(mask)
        out_mask[size:, size:] = (
            is_to_be_masked_corners[:-size, :-size]
            + is_to_be_masked_corners[size:, size:]
            - is_to_be_masked_corners[:-size, size:]
            - is_to_be_masked_corners[size:, :-size]
        )
...
def ShapeCompletionL1(
    mask,
    init_l1_thresh=0.9,
    thresh_decay=0.7,
    break_its=15,
    square_sizes=[100, 75, 50, 25],
    soft=False,
):
    if break_its == 0:
        ...
        if soft:
            combined_masks = ThresholdSTEGEFunction.apply(
                torch.stack(
                    list(
                        [
                            ShapeCompletionL1KnownSquare(
                                mask, x, init_l1_thresh, soft=True
                            )
            ...
            if combined_masks.sum() > 0:
                ...
                return ShapeCompletionL1(
                    mask,
                    init_l1_thresh=init_l1_thresh * thresh_decay,
                    thresh_decay=thresh_decay,
                    break_its=break_its - 1,
            ...
            combined_masks = ThresholdSTEGEFunction.apply(
                torch.stack(
                    list(
                        [
                            ShapeCompletionL1KnownSquare(mask, x, init_l1_thresh)
                            for x in square_sizes
                        ]
                    )
                ).sum(dim=0)
...
def ShapeCompletionMultiPatch(
    ...
    for i in range(n_patch):
        ...
        patch_i_completed = ShapeCompletionL1(
            patch_i,
            init_l1_thresh=init_l1_thresh,
            thresh_decay=thresh_decay,
            break_its=break_its,
            square_sizes=square_sizes,
            soft=soft,
...
class PatchDetector(torch.nn.Module):
    def __init__(
        ...
        if unet:
            ...
            self.unet = UNet(
                n_channels, n_classes, bilinear=bilinear, base_filter=base_filter
            )
    ...
    def forward(
        ...
        for x in image_list:
            ...
            if shape_completion and not simple_shape_completion:
                if self.n_patch > 1:
                    mask = ShapeCompletionMultiPatch(
                        mask, n_patch=self.n_patch, soft=soft_shape_completion
                    )
                else:
