        ...
        for cnt_frame in range(frame_count_org):
            ...
            if mask_path is not None:
                ...
                _, binary_mask = cv2.threshold(
                    masks, 1, 255, cv2.THRESH_BINARY
                )  # obtain binary mask only
...
def preprocess(dataset_path, mask_path, mode, num_frames, stride, logger):
    ...
    movies_path_list = sorted(
        [
            Path(p)
            for p in glob.glob(os.path.join(dataset_path, "**/*.mp4"), recursive=True)
        ]
    )
    ...
    if mask_path is not None:
        masks_path_list = sorted(
            [
                Path(p)
                for p in glob.glob(os.path.join(mask_path, "**/*.mp4"), recursive=True)
            ]
        )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_processes) as executor:
        ...
        for movie_path in movies_path_list:
            ...
            if mask_path is not None:
                ...
                mask_path = next(
                    (path for path in masks_path_list if path.stem == movie_path.stem),
                    None,
                )
        ...
        for future in tqdm(
            concurrent.futures.as_completed(futures), total=len(movies_path_list)
        ):
...
if __name__ == "__main__":
    ...
    if dataset_name == "FaceForensics++":
        ...
        sub_dataset_paths = [
            Path(os.path.join(dataset_path, name, comp)) for name in sub_dataset_names
        ]
        # mask
        mask_dataset_names = [
            "manipulated_sequences/Deepfakes",
        ...
        # mask_dataset_names = []
        mask_dataset_paths = [
            Path(os.path.join(dataset_path, name)) for name in mask_dataset_names
        ]
    ## Celeb-DF-v1
    elif dataset_name == "Celeb-DF-v1":
        sub_dataset_names = ["Celeb-real", "Celeb-synthesis", "YouTube-real"]
        sub_dataset_paths = [
            Path(os.path.join(dataset_path, name)) for name in sub_dataset_names
        ]
    ...
    ## DFDC-test
        ...
        sub_train_dataset_paths = [
            Path(os.path.join(dataset_path, "train", name))
            for name in sub_train_dataset_names
        ]
        sub_dataset_paths = [
    ...
    if "sub_dataset_paths" in globals() and len(sub_dataset_paths) != 0:
        ...
        for sub_dataset_path in sub_dataset_paths:
            ...
            if (
                ...
                and sub_dataset_path.parent in mask_dataset_paths
            ):
